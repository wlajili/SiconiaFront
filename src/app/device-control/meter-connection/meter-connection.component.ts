import { Compteur } from 'src/app/compteur';
...
export class MeterConnectionComponent implements OnInit {
    ...
    text$.pipe(
      ...
      map(term => {
        if (term.length < 3) {
          ...
          this.testgetMeters().subscribe((data) => {
            console.log(data);
            this.listMrid = data;
            });
            console.log( this.listMrid);
            return this.listMrid.filter(v => v.toLowerCase().indexOf(term.toLowerCase()) > -1).slice(0, 10);
            }
          }
        ))

  testgetMeters() {
      return this.meterService.getAllMeterMridStartsWith(this.idCompteur.toUpperCase());
  }
  ...
  saveEchange() {
    ...
    this.echange.verb = 'MeterConnectArmed';
